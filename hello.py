...
def main():#principal script for parsing command-line arguments, reading annotation file and genomic data-output file
    ...
    if args.output_type not in ["name", "position"]:# Checking if the output type is valid and in format
    ...
    with open(args.output, "w") as output_file: #the output file is specified by the -o argument
        # iterating through genes in the annotation data and checks for the presence of a PAM sequence upstream of each gene.
...
def is_pam_upstream(gene_annotation, genome_seq, upstream):
    # Get the chromosome sequence for the gene
...
def get_pam_upstream_position(gene_annotation, genome_seq, upstream):
    ...
    pam_plus_strand_index = utils.get_pam(upstream_seq)
...
def is_pam_upstream(gene_annotation, genome_seq, upstream):
    return get_pam_upstream_position(gene_annotation, genome_seq, upstream) is not None
